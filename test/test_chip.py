...
class ChipOspiMaster:
    ...
    async def burst_stream_write(self, addr: int, data: list[int]) -> None:
        """run one streaming OSPI write burst: cmd+addr then N data bytes."""
        bytes_out = [
            0x02,
            (addr >> 16) & 0xFF,
            (addr >>  8) & 0xFF,
    ...
    async def burst_stream_read(self, addr: int, nbytes: int) -> list[int]:
        """run one streaming OSPI read burst and return N data bytes."""
        header = [
            0x03,
            (addr >> 16) & 0xFF,
            (addr >>  8) & 0xFF,
        ...
        for _ in range(nbytes):
            ...
            self.setSck(1)
            await ClockCycles(self.dut.clk, SCK_HALF_CYCLES)
            rbytes.append(int(self.dut.uio_out.value) & 0xFF)
    ...
    async def loadPageStreaming(self, instrs: list[int]) -> None:
        """load a page using one streaming OSPI write burst of 8 bytes."""
        nop = ocpu_asm.NOP_WORD
        page = (instrs + [nop] * SLOTS_PER_PAGE)[:SLOTS_PER_PAGE]
        bytes_out: list[int] = []
        for w in page:
            bytes_out.append(w & 0xFF)
            bytes_out.append((w >> 8) & 0xFF)

        self.setPageLoading(1)
        await ClockCycles(self.dut.clk, 2)
        await self.burst_stream_write(0x000000, bytes_out)
        self.setPageDone(1)
        await ClockCycles(self.dut.clk, 2)
        self.setPageDone(0)
        self.setPageLoading(0)
...
@cocotb.test()
async def test_chip_ospi_streaming_load_and_run(dut):
    """end-to-end OSPI integration test using a streaming page load burst."""
    ...
    tmpPath = os.path.join(HERE, '_tmp_chip_prog_stream.s')
    with open(tmpPath, 'w') as f:
        f.write(src)
    try:
        asm = ocpu_asm.assembleFile(tmpPath)
    ...
    await m.loadPageStreaming(page0)
    ...
    assert halted, "chip did not reach HLT within deadline cycles"
    dut._log.info(
        f"chip halted after streaming OSPI page load (uo_out={int(dut.uo_out.value):08b})")
...
@cocotb.test()
async def test_chip_ospi_streaming_readback(dut):
    """write 4 instruction words via one streaming burst, then read them
    back via a streaming read burst."""
    ...
    m.setPageLoading(1)
    await ClockCycles(dut.clk, 4)
    await m.burst_stream_write(0x000000, bytes_out)

    rbytes = await m.burst_stream_read(0x000000, len(bytes_out))
    for slot, expected in enumerate(words):
        lo = rbytes[slot * 2]
        hi = rbytes[slot * 2 + 1]
        got = (hi << 8) | lo
        assert got == expected, (
            f"iRAM slot {slot} streaming readback mismatch: got 0x{got:04x}, "
            f"expected 0x{expected:04x} (lo=0x{lo:02x}, hi=0x{hi:02x})"
        )
